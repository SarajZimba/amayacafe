...
class User(AbstractUser, BaseModel):
    ...
    def save(self, *args, **kwargs):
        if not self.email:
            self.email = f"{self.username}@silverline.com"
...
class Customerlogin(BaseModel, AbstractUser):
    ...
    groups = models.ManyToManyField(
        "auth.Group",
        related_name="customerlogin_groups",
        blank=True,
    ...
    user_permissions = models.ManyToManyField(
        "auth.Permission",
        related_name="customerlogin_user_permissions",
        blank=True,
        help_text="Specific permissions for this user.",
        verbose_name="user permissions", null=True
    )
...
class CustomerNormalLogin(models.Model):
