...
from .models import ProductCategory, ItemReconcilationApiItem, RecipieItemSale
...
class ReconcileView(View): 
    ...
    def get(self, request):
        ...
        if not exists_in_bst:
            ...
            recepie_items = RecipieItemSale.objects.filter(transaction_date=filter_date, branch=filter_branch)
            ...
            for item in recepie_items:
                product_id = str(item.product.id)
                product = new_products.get(product_id, None)
                if product:
                    has_sold = product.get('sold', None)
                    if has_sold:
...
from .forms import ProductRecipieForm
class ProductRecipieMixin:
    model = ProductRecipie
    form_class = ProductRecipieForm
    paginate_by = 10
    queryset = ProductRecipie.objects.filter(status=True,is_deleted=False)
    success_url = reverse_lazy('productrecipie_list')

class ProductRecipieList(ProductRecipieMixin, ListView):
    template_name = "productrecipie/productrecipie_list.html"
    queryset = ProductRecipie.objects.filter(status=True,is_deleted=False)

    def get_context_data(self, **kwargs: Any) -> Dict[str, Any]:
        context = super().get_context_data(**kwargs)
        recipies = context['object_list']
        new_data = []

        for recipie in recipies:
            data = {'pk':recipie.pk,'title':recipie.product.title,'content':""}
            for item in recipie.items:
                item_id = item['id']
                if Product.objects.filter(pk=item_id).exists():
                    product = Product.objects.get(pk=item_id)
                    data['content']+=f"{product.title}({item['quantity']}), "
            new_data.append(data)
        context['object_list']= new_data
        return context

class ProductRecipieDetail(ProductRecipieMixin, DetailView):
    template_name = "productrecipie/productrecipie_detail.html"

class ProductRecipieCreate(ProductRecipieMixin, CreateView):
    template_name = "productrecipie/productrecipie_create.html"
    ...
    def post(self, request: HttpRequest, *args: str, **kwargs: Any) -> HttpResponse:
        ...
        if product_id and items:
            try:
                ...
                pass
        return redirect(reverse_lazy('productrecipie_list'))

class ProductRecipieUpdate(ProductRecipieMixin, UpdateView):
 
    template_name = "update.html"
    # template_name = "update.html"

# def productrecipie_update(request, pk):
#     productrecipie = get_object_or_404(ProductRecipie, pk=pk)

#     if request.method == 'POST':
#         form = ProductRecipieForm(request.POST, instance=productrecipie)
#         if form.is_valid():
#             form.save()
#             return redirect('productrecipie_list')
#     else:
#         form = ProductRecipieForm(instance=productrecipie)

    # return redirect('productrecipie_list')

def productrecipie_delete(request, pk):
    # Retrieve the object to be deleted using the primary key (pk)
    productrecipie = get_object_or_404(ProductRecipie, pk=pk)

    # Perform the delete operation
    productrecipie.delete()

    # Redirect to the success URL after deletion (e.g., a list view)
    return redirect('productrecipie_list')